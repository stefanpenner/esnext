...
function transform(ast, options) {
  ...
  if (options.computedPropertyKeys !== false) {
    es6computedProperties.transform(ast);
  }

  if (options.arrowFunction !== false) {
    es6arrowfn.transform(ast);
  }

  //if (options['class'] !== false) {
  //  es6class.transform(ast);
  //}

  if (options.rest !== false) {
    es6restParams.transform(ast);
  }

  if (options.defaultParams !== false) {
    es6defaultParams.transform(ast);
  }
  ...
  //  regexpu.transform(ast);
  //}

  if (options.spread !== false) {
    es6spread.transform(ast);
  }

  if (options.templates !== false) {
    es6templates.transform(ast);
  }
  ...
  if (options.destructuring !== false) {
    es6destructuring.transform(ast);
  }

  if (options.objectShorthand !== false) {
    es6objectShort.transform(ast);
  }

  if (options.objectConcise !== false) {
    es6objectConcise.transform(ast);
  }
